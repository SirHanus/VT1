...
from gui_qt import QtCore, QtGui, QtWidgets
...
class ClusteringTab(QtWidgets.QWidget):
    ...
    # ---------- Process helpers ----------
    def _start(self, script_rel: str, args: list[str], widgets: dict):
        if widgets.get('proc') is not None:
            QtWidgets.QMessageBox.warning(self, 'Already running', 'Stop the current process first.')
            return
        py = sys.executable or 'python'
        script = Path(__file__).resolve().parent / 'team_clustering' / script_rel
        ...
        try:
            ...
            widgets['log'].appendPlainText(f'[GUI] Failed to start: {e}');
            widgets['proc'] = None
    ...
    def _stop(self, widgets: dict):
        ...
        if p is not None:
            ...
            p.terminate()
            if not p.waitForFinished(3000): widgets['log'].appendPlainText(
                '[GUI] Killing…'); p.kill(); p.waitForFinished(2000)
        widgets['proc'] = None
    ...
    def _make_build_tab(self) -> QtWidgets.QWidget:
        ...
        ed_videos_dir = QtWidgets.QLineEdit(str(root / 'videos_all' / 'CAR_vs_NYR'));
        ed_videos_dir.setToolTip('Directory with input videos (*.mp4) (--videos-dir)')
        btn_videos_dir = QtWidgets.QPushButton('Browse…');
        btn_videos_dir.setToolTip('Pick videos directory')
        btn_videos_dir.clicked.connect(lambda: self._pick_dir_into(ed_videos_dir))
        form.addRow('Videos dir (--videos-dir)', self._row(ed_videos_dir, btn_videos_dir))
        ed_glob = QtWidgets.QLineEdit('*.mp4');
        ...
        ed_yolo_model = QtWidgets.QLineEdit(str(root / 'offline_pipeline' / 'yolo11n.pt'));
        ...
        ds_fps.setValue(1.0);
        ds_fps.setToolTip('Target frames per second to sample (default 1 FPS) (--fps)')
        form.addRow('FPS (--fps)', ds_fps)
        sb_max_seconds = QtWidgets.QSpinBox();
        sb_max_seconds.setRange(0, 36000);
        sb_max_seconds.setToolTip('Limit seconds per video (0=all) (--max-seconds)')
        ...
        ds_central.setValue(0.6);
        ds_central.setToolTip('Fraction of bbox width/height to keep around center (0<r<=1) (--central-ratio)')
        form.addRow('Central ratio (--central-ratio)', ds_central)
        sb_min_crop = QtWidgets.QSpinBox();
        sb_min_crop.setRange(1, 2048);
        sb_min_crop.setValue(32);
        sb_min_crop.setToolTip('Discard crops smaller than this (pixels) (--min-crop-size)')
        form.addRow('Min crop size (--min-crop-size)', sb_min_crop)
        ed_siglip = QtWidgets.QLineEdit('google/siglip-base-patch16-224');
        ed_siglip.setToolTip('SigLIP Vision model with projection (HF id) (--siglip)')
        form.addRow('SigLIP (--siglip)', ed_siglip)
        sb_batch = QtWidgets.QSpinBox();
        sb_batch.setRange(1, 1024);
        sb_batch.setValue(64);
        sb_batch.setToolTip('Embedding batch size (--batch)')
        form.addRow('Batch size (--batch)', sb_batch)
        cb_device = QtWidgets.QComboBox();
        cb_device.addItems(['cuda', 'cpu']);
        cb_device.setToolTip('Inference device (--device)')
        form.addRow('Device (--device)', cb_device)
        ed_out_dir = QtWidgets.QLineEdit(str(root / 'offline_pipeline' / 'team_clustering' / 'clustering'));
        ed_out_dir.setToolTip('Output directory root (default: team_clustering/clustering) (--out-dir)')
        btn_out_dir = QtWidgets.QPushButton('Browse…');
        btn_out_dir.setToolTip('Pick output folder');
        ...
        cb_save_crops = QtWidgets.QCheckBox('Save crop images');
        cb_save_crops.setToolTip('Save crop images to disk (--save-crops)')
        form.addRow('Save crops (--save-crops)', cb_save_crops)
        sb_seed = QtWidgets.QSpinBox();
        sb_seed.setRange(0, 10_000);
        sb_seed.setToolTip('Random seed for reproducibility (--seed)')
        form.addRow('Seed (--seed)', sb_seed)
        cb_verbose = QtWidgets.QCheckBox('Verbose logging');
        cb_verbose.setToolTip('Enable verbose logging (--verbose)')
        form.addRow('Verbose (--verbose)', cb_verbose)
        lay.addLayout(form)
        # Run
        run = QtWidgets.QHBoxLayout();
        btn_run = QtWidgets.QPushButton('Run Build');
        ...
        btn_run.clicked.connect(lambda: self._start('build_training_set.py', self._args_build(
    ...
    def _make_cluster_tab(self) -> QtWidgets.QWidget:
        ...
        lay = QtWidgets.QVBoxLayout(w);
        form = QtWidgets.QFormLayout()
        base = Path(__file__).resolve().parent / 'team_clustering' / 'clustering'
        ed_in_root = QtWidgets.QLineEdit(str(base));
        ed_in_root.setToolTip(
            'Root directory with per-video embeddings (default: team_clustering/clustering) (--in-root)')
        ...
        ed_out_root = QtWidgets.QLineEdit(str(base));
        ed_out_root.setToolTip('Output directory for clustering artifacts (--out-dir)')
        btn_out_root = QtWidgets.QPushButton('Browse…');
        btn_out_root.setToolTip('Pick output dir');
        btn_out_root.clicked.connect(lambda: self._pick_dir_into(ed_out_root))
        form.addRow('Out dir (--out-dir)', self._row(ed_out_root, btn_out_root))
        sb_k = QtWidgets.QSpinBox();
        sb_k.setRange(2, 32);
        sb_k.setValue(2);
        sb_k.setToolTip('Number of clusters for KMeans (e.g., 2 teams) (--k)')
        form.addRow('K (--k)', sb_k)
        sb_umap_dim = QtWidgets.QSpinBox();
        sb_umap_dim.setRange(2, 512);
        sb_umap_dim.setValue(16);
        sb_umap_dim.setToolTip('UMAP output dimensionality (--umap-dim)')
        form.addRow('UMAP dim (--umap-dim)', sb_umap_dim)
        sb_umap_neighbors = QtWidgets.QSpinBox();
        sb_umap_neighbors.setRange(2, 200);
        sb_umap_neighbors.setValue(15);
        sb_umap_neighbors.setToolTip('UMAP n_neighbors (--umap-neighbors)')
        form.addRow('UMAP neighbors (--umap-neighbors)', sb_umap_neighbors)
        ed_umap_metric = QtWidgets.QLineEdit('cosine');
        ed_umap_metric.setToolTip('UMAP distance metric (--umap-metric)')
        form.addRow('UMAP metric (--umap-metric)', ed_umap_metric)
        ds_umap_min_dist = QtWidgets.QDoubleSpinBox();
        ds_umap_min_dist.setRange(0.0, 1.0);
        ds_umap_min_dist.setSingleStep(0.05);
        ds_umap_min_dist.setValue(0.1);
        ds_umap_min_dist.setToolTip('UMAP min_dist (lower=more compact clusters) (--umap-min-dist)')
        form.addRow('UMAP min_dist (--umap-min-dist)', ds_umap_min_dist)
        ed_reuse_umap = QtWidgets.QLineEdit('');
        ed_reuse_umap.setToolTip('Path to a pre-trained umap.pkl to reuse (skip fitting) (--reuse-umap)')
        btn_reuse_umap = QtWidgets.QPushButton('Browse…');
        btn_reuse_umap.setToolTip('Pick existing umap.pkl');
        btn_reuse_umap.clicked.connect(lambda: self._pick_file_into(ed_reuse_umap, 'UMAP (*.pkl);;All (*)'))
        form.addRow('Reuse UMAP (--reuse-umap)', self._row(ed_reuse_umap, btn_reuse_umap))
        sb_limit = QtWidgets.QSpinBox();
        sb_limit.setRange(0, 10_000_000);
        sb_limit.setToolTip('Limit N total rows (0=all) (--limit)')
        form.addRow('Limit rows (--limit)', sb_limit)
        cb_plot = QtWidgets.QCheckBox('Save scatter plot');
        cb_plot.setToolTip('Save a 2D scatter plot if possible (--plot)')
        form.addRow('Plot (--plot)', cb_plot)
        sb_seed2 = QtWidgets.QSpinBox();
        sb_seed2.setRange(0, 10_000);
        sb_seed2.setToolTip('Random seed (--seed)')
        form.addRow('Seed (--seed)', sb_seed2)
        cb_save_models = QtWidgets.QCheckBox('Persist UMAP & KMeans');
        cb_save_models.setToolTip('Persist fitted UMAP and KMeans models (umap.pkl, kmeans.pkl) (--save-models)')
        form.addRow('Save models (--save-models)', cb_save_models)
        lay.addLayout(form)
        run = QtWidgets.QHBoxLayout();
        btn_run = QtWidgets.QPushButton('Run Cluster');
        btn_stop = QtWidgets.QPushButton('Stop');
        btn_stop.setEnabled(False);
        status = QtWidgets.QLabel('Idle');
        pb = QtWidgets.QProgressBar();
        ...
        lay.addWidget(log, 1)
        widgets = {'proc': None, 'run': btn_run, 'stop': btn_stop, 'status': status, 'progress': pb, 'log': log}
        btn_run.clicked.connect(lambda: self._start('cluster_umap_kmeans.py', self._args_cluster(
            ed_in_root.text(), ed_out_root.text(), sb_k.value(), sb_umap_dim.value(), sb_umap_neighbors.value(),
            ed_umap_metric.text(), ds_umap_min_dist.value(), ed_reuse_umap.text(), sb_limit.value(),
            cb_plot.isChecked(), sb_seed2.value(), cb_save_models.isChecked()
        ), widgets))
        btn_stop.clicked.connect(lambda: self._stop(widgets))
        return w

    def _args_cluster(self, in_root, out_root, k, dim, neighbors, metric, min_dist, reuse_umap, limit, plot, seed,
                      save_models):
        args = []
        ...
        add('--in-root', in_root);
        add('--out-dir', out_root);
        add('--k', k);
        add('--umap-dim', dim);
        add('--umap-neighbors', neighbors);
        add('--umap-metric', metric);
        add('--umap-min-dist', min_dist)
    ...
    def _make_audit_tab(self) -> QtWidgets.QWidget:
        ...
        btn_in_root.clicked.connect(lambda: self._pick_dir_into(ed_in_root))
        form.addRow('In root (--in-root)', self._row(ed_in_root, btn_in_root))
        ed_out_dir = QtWidgets.QLineEdit(str(Path(__file__).resolve().parent / 'team_clustering' / 'outputs'));
        ...
        sb_per_video.setToolTip('Max crops per video to include in mosaic (--per-video)')
        ...
        lay.addWidget(log, 1)
        widgets = {'proc': None, 'run': btn_run, 'stop': btn_stop, 'status': status, 'progress': pb, 'log': log}
        btn_run.clicked.connect(lambda: self._start('audit_training_set.py',
                                                    self._args_audit(ed_in_root.text(), ed_out_dir.text(),
                                                                     sb_per_video.value(), cb_save_grid.isChecked(),
                                                                     sb_seed.value()), widgets))
        btn_stop.clicked.connect(lambda: self._stop(widgets))
        return w
    ...
    def _make_eval_tab(self) -> QtWidgets.QWidget:
        ...
        ed_video.setToolTip('Optional: sample frames from a video file (--video)')
        btn_video = QtWidgets.QPushButton('Browse…');
        btn_video.setToolTip('Pick video file');
        btn_video.clicked.connect(lambda: self._pick_file_into(ed_video, 'Video (*.mp4 *.avi *.mkv);;All (*)'))
        form.addRow('Video (--video)', self._row(ed_video, btn_video))
        sb_frame_step = QtWidgets.QSpinBox();
        sb_frame_step.setRange(1, 10_000);
        sb_frame_step.setValue(30);
        sb_frame_step.setToolTip('Take 1 frame every N frames when reading a video (--frame-step)')
        ...
        ed_team_models = QtWidgets.QLineEdit(str(root / 'offline_pipeline' / 'team_clustering' / 'clustering'));
        ...
        form.addRow('SigLIP (--siglip)', ed_siglip)
        ed_yolo_model = QtWidgets.QLineEdit(str(root / 'offline_pipeline' / 'yolo11n.pt'));
        ed_yolo_model.setToolTip('YOLO detection model path/id (--yolo-model)')
        btn_yolo_model = QtWidgets.QPushButton('Browse…');
        btn_yolo_model.setToolTip('Pick YOLO model file');
        btn_yolo_model.clicked.connect(lambda: self._pick_file_into(ed_yolo_model, 'Model (*.pt *.onnx);;All (*)'))
        form.addRow('YOLO model (--yolo-model)', self._row(ed_yolo_model, btn_yolo_model))
        sb_imgsz = QtWidgets.QSpinBox();
        sb_imgsz.setRange(64, 4096);
        sb_imgsz.setValue(640);
        ...
        ds_conf.setValue(0.30);
        ds_conf.setToolTip('YOLO confidence threshold (--conf)')
        form.addRow('Conf (--conf)', ds_conf)
        sb_max_boxes = QtWidgets.QSpinBox();
        sb_max_boxes.setRange(1, 100);
        sb_max_boxes.setValue(8);
        sb_max_boxes.setToolTip('Max boxes per image/frame to annotate (--max-boxes)')
        form.addRow('Max boxes (--max-boxes)', sb_max_boxes)
        ds_central = QtWidgets.QDoubleSpinBox();
        ds_central.setRange(0.05, 1.0);
        ds_central.setSingleStep(0.05);
        ds_central.setValue(0.6);
        ds_central.setToolTip('Central crop ratio of bbox (--central-ratio)')
        form.addRow('Central ratio (--central-ratio)', ds_central)
        cb_device = QtWidgets.QComboBox();
        cb_device.addItems(['cuda', 'cpu']);
        ...
        sb_limit_images.setValue(50);
        sb_limit_images.setToolTip('Max annotated images to write (--limit-images)')
        ...
        lay.addWidget(log, 1)
        widgets = {'proc': None, 'run': btn_run, 'stop': btn_stop, 'status': status, 'progress': pb, 'log': log}
        btn_run.clicked.connect(lambda: self._start('eval_clustering.py', self._args_eval(
            ed_images_dir.text(), ed_glob.text(), ed_video.text(), sb_frame_step.value(), sb_max_frames.value(),
            ed_team_models.text(), ed_siglip.text(), ed_yolo_model.text(), sb_imgsz.value(), ds_conf.value(),
            sb_max_boxes.value(), ds_central.value(), cb_device.currentText(), ed_out_dir.text(), cb_show.isChecked(),
            cb_save_grid.isChecked(), sb_limit_images.value()
        ), widgets))
        btn_stop.clicked.connect(lambda: self._stop(widgets))
        return w

    def _args_eval(self, images_dir, glob, video, frame_step, max_frames, team_models, siglip, yolo_model, imgsz, conf,
                   max_boxes, central_ratio, device, out_dir, show, save_grid, limit_images):
        args = []
        ...
        add('--team-models', team_models);
        add('--siglip', siglip);
        add('--yolo-model', yolo_model);
        add('--imgsz', imgsz);
        add('--conf', conf);
        add('--max-boxes', max_boxes);
        add('--central-ratio', central_ratio);
        add('--device', device);
        add('--out-dir', out_dir);
        add('--limit-images', limit_images)
    ...
    def _pick_dir_into(self, le: QtWidgets.QLineEdit):
        path = QtWidgets.QFileDialog.getExistingDirectory(self, 'Select directory',
                                                          le.text() or str(Path('.').resolve()))
        if path: le.setText(path)

    def _pick_file_into(self, le: QtWidgets.QLineEdit, filter_str: str):
        path, _ = QtWidgets.QFileDialog.getOpenFileName(self, 'Select file', le.text() or str(Path('.').resolve()),
                                                        filter_str)
        if path: le.setText(path)
