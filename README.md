...
dist\vt1-gui.exe
```
If the GUI detects `vt1-pipeline.exe` in the same folder it will invoke it directly for pipeline runs; otherwise it falls back to the embedded Python module.
...
- `models/team_clustering/umap.pkl`, `models/team_clustering/kmeans.pkl` (team clustering)
