...
class ClusteringTab(QtWidgets.QWidget):
    ...
    def _make_build_tab(self) -> QtWidgets.QWidget:
        ...
        cfg = settings();
        root = cfg.repo_root
        ed_videos_dir = QtWidgets.QLineEdit(str(cfg.training_videos_dir));
        ...
        ds_fps.setValue(float(cfg.build_fps));
        ds_fps.setToolTip('Target frames per second to sample (--fps)')
        ...
        ds_central.setToolTip('Fraction of bbox width/height to keep around center (--central-ratio)')
        form.addRow('Central ratio (--central-ratio)', ds_central)
        sb_min_crop = QtWidgets.QSpinBox();
        sb_min_crop.setRange(1, 2048);
        sb_min_crop.setValue(int(cfg.build_min_crop_size));
        sb_min_crop.setToolTip('Discard crops smaller than this (--min-crop-size)')
        form.addRow('Min crop size (--min-crop-size)', sb_min_crop)
        ed_siglip = QtWidgets.QLineEdit(str(cfg.siglip_model));
        ed_siglip.setToolTip('SigLIP Vision model id (--siglip)')
        form.addRow('SigLIP (--siglip)', ed_siglip)
        sb_batch = QtWidgets.QSpinBox();
        sb_batch.setRange(1, 1024);
        sb_batch.setValue(int(cfg.build_batch_size));
        ...
        sb_seed.setValue(int(cfg.random_seed));
    ...
    def _make_cluster_tab(self) -> QtWidgets.QWidget:
        ...
        ed_in_root.setToolTip('Root directory with per-video embeddings (--in-root)')
        ...
        sb_k.setRange(2, 32);
        sb_k.setValue(int(cfg.cluster_k));
        sb_k.setToolTip('Number of clusters (--k)')
        form.addRow('K (--k)', sb_k)
        sb_umap_dim = QtWidgets.QSpinBox();
        sb_umap_dim.setRange(2, 512);
        sb_umap_dim.setValue(int(cfg.umap_dim));
        ...
        ds_umap_min_dist.setValue(float(cfg.umap_min_dist));
        ds_umap_min_dist.setToolTip('UMAP min_dist (--umap-min-dist)')
    ...
    def _make_audit_tab(self) -> QtWidgets.QWidget:
        ...
        lay.addWidget(log, 1)
        widgets = {'proc': None, 'run': btn_run, 'stop': btn_stop, 'status': status, 'progress': pb, 'log': log}
        btn_run.clicked.connect(lambda: self._start('vt1.team_clustering.audit_training_set',
    ...
    def _make_eval_tab(self) -> QtWidgets.QWidget:
        ...
        def _run_eval():
            ...
            self._start('vt1.team_clustering.eval_clustering', self._args_eval(
                ...
                sb_max_boxes.value(), ds_central.value(), cb_device.currentText(), ed_out_dir.text(),
                cb_show.isChecked(), cb_save_grid.isChecked(), sb_limit_images.value()
            ), widgets)
