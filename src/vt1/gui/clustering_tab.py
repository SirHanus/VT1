...
class ClusteringTab(QtWidgets.QWidget):
    ...
    def _stop(self, widgets: dict):
        ...
        if p is not None:
            ...
            if not p.waitForFinished(3000):
                widgets["log"].appendPlainText("[GUI] Killing…")
                p.kill()
    ...
    def _handle_stream(self, text: str, widgets: dict):
        for seg in text.split("\r"):
            ...
            if any(
                k in seg
                for k in ("Processing", "Done", "Complete", "Finished", "Output")
            ):
                widgets["status"].setText(seg.strip()[:160])
            widgets["log"].appendPlainText(seg)

    def _on_finished(
        self, code: int, status: QtCore.QProcess.ExitStatus, widgets: dict
    ):
    ...
    def _make_build_tab(self) -> QtWidgets.QWidget:
        ...
        form.addRow(
            "Videos dir (--videos-dir)", self._row(ed_videos_dir, btn_videos_dir)
        )
        ...
        ds_central.setToolTip(
            "Fraction of bbox width/height to keep around center (--central-ratio)"
        )
        ...
        btn_run.clicked.connect(
            lambda: self._start(
                "vt1.team_clustering.build_training_set",
                self._args_build(
                    ed_videos_dir.text(),
                    ed_glob.text(),
                    ed_yolo_model.text(),
                    ds_det_thr.value(),
                    ds_fps.value(),
                    sb_max_seconds.value(),
                    ds_central.value(),
                    sb_min_crop.value(),
                    ed_siglip.text(),
                    sb_batch.value(),
                    cb_device.currentText(),
                    ed_out_dir.text(),
                    cb_save_crops.isChecked(),
                    sb_seed.value(),
                    cb_verbose.isChecked(),
                ),
                widgets,
            )
        )
    ...
    def _make_cluster_tab(self) -> QtWidgets.QWidget:
        ...
        ed_reuse_umap.setToolTip(
            "Path to a pre-trained umap.pkl to reuse (skip fitting) (--reuse-umap)"
        )
        btn_reuse_umap = QtWidgets.QPushButton("Browse…")
        btn_reuse_umap.setToolTip("Pick existing umap.pkl")
        btn_reuse_umap.clicked.connect(
            lambda: self._pick_file_into(ed_reuse_umap, "UMAP (*.pkl);;All (*)")
        )
        form.addRow(
            "Reuse UMAP (--reuse-umap)", self._row(ed_reuse_umap, btn_reuse_umap)
        )
        ...
        cb_save_models.setToolTip(
            "Persist fitted UMAP and KMeans models (umap.pkl, kmeans.pkl) (--save-models)"
        )
        ...
        btn_run.clicked.connect(
            lambda: self._start(
                "vt1.team_clustering.cluster_umap_kmeans",
                self._args_cluster(
                    ed_in_root.text(),
                    ed_out_root.text(),
                    sb_k.value(),
                    sb_umap_dim.value(),
                    sb_umap_neighbors.value(),
                    ed_umap_metric.text(),
                    ds_umap_min_dist.value(),
                    ed_reuse_umap.text(),
                    sb_limit.value(),
                    cb_plot.isChecked(),
                    sb_seed2.value(),
                    cb_save_models.isChecked(),
                ),
                widgets,
            )
        )
    ...
    def _make_audit_eval_tab(self) -> QtWidgets.QWidget:
        ...
        def _run_eval():
            if not self._preflight_eval_models(ed_team_models.text()):
                return
            self._start(
                "vt1.team_clustering.eval_clustering",
                self._args_eval(
                    ed_images_dir.text(),
                    ed_glob.text(),
                    ed_video.text(),
    ...
    def _preflight_eval_models(self, models_dir: str) -> bool:
        try:
            ...
            QtWidgets.QMessageBox.warning(
                self, "Check failed", f"Error while checking team models: {e}"
            )
            return False
    ...
    def _args_eval(
        ...
        if images_dir:
            add("--images-dir", images_dir)
            add("--glob", glob)
        if video:
            add("--video", video)
            add("--frame-step", frame_step)
            add("--max-frames", max_frames)
    ...
    def _pick_dir_into(self, le: QtWidgets.QLineEdit):
        path = QtWidgets.QFileDialog.getExistingDirectory(
            self, "Select directory", le.text() or str(Path(".").resolve())
        )
        if path:
            le.setText(path)
    ...
    def _pick_file_into(self, le: QtWidgets.QLineEdit, filter_str: str):
        path, _ = QtWidgets.QFileDialog.getOpenFileName(
            self, "Select file", le.text() or str(Path(".").resolve()), filter_str
        )
        if path:
            le.setText(path)
