...
class ClusteringTab(QtWidgets.QWidget):
    ...
    def _make_build_tab(self) -> QtWidgets.QWidget:
        ...
        ed_videos_dir = QtWidgets.QLineEdit(str(cfg.training_videos_dir)); ed_videos_dir.setToolTip('Directory with input videos (*.mp4) (--videos-dir)')
        ...
        ed_glob = QtWidgets.QLineEdit(str(cfg.videos_glob)); ed_glob.setToolTip('Glob to match videos inside --videos-dir (--glob)')
        ...
        ds_det_thr = QtWidgets.QDoubleSpinBox(); ds_det_thr.setRange(0.0,1.0); ds_det_thr.setSingleStep(0.01); ds_det_thr.setValue(float(cfg.det_score_thr_default)); ds_det_thr.setToolTip('Score threshold for detections (--det-score-thr)')
        form.addRow('Det score thr (--det-score-thr)', ds_det_thr)
        ed_person = QtWidgets.QLineEdit(str(cfg.person_class_name)); ed_person.setToolTip('Class name to treat as player/person (--person-class-name)')
        ...
        ds_fps = QtWidgets.QDoubleSpinBox(); ds_fps.setRange(0.1,60.0); ds_fps.setSingleStep(0.1); ds_fps.setValue(float(cfg.build_fps)); ds_fps.setToolTip('Target frames per second to sample (--fps)')
        ...
        ds_central = QtWidgets.QDoubleSpinBox(); ds_central.setRange(0.05,1.0); ds_central.setSingleStep(0.05); ds_central.setValue(float(cfg.central_ratio_default)); ds_central.setToolTip('Fraction of bbox width/height to keep around center (--central-ratio)')
        form.addRow('Central ratio (--central-ratio)', ds_central)
        sb_min_crop = QtWidgets.QSpinBox(); sb_min_crop.setRange(1,2048); sb_min_crop.setValue(int(cfg.build_min_crop_size)); sb_min_crop.setToolTip('Discard crops smaller than this (--min-crop-size)')
        form.addRow('Min crop size (--min-crop-size)', sb_min_crop)
        ed_siglip = QtWidgets.QLineEdit(str(cfg.siglip_model)); ed_siglip.setToolTip('SigLIP Vision model id (--siglip)')
        form.addRow('SigLIP (--siglip)', ed_siglip)
        sb_batch = QtWidgets.QSpinBox(); sb_batch.setRange(1,1024); sb_batch.setValue(int(cfg.build_batch_size)); sb_batch.setToolTip('Embedding batch size (--batch)')
        ...
        sb_seed = QtWidgets.QSpinBox(); sb_seed.setRange(0,10_000); sb_seed.setValue(int(cfg.random_seed)); sb_seed.setToolTip('Random seed (--seed)')
    ...
    def _make_cluster_tab(self) -> QtWidgets.QWidget:
        w = QtWidgets.QWidget(); lay = QtWidgets.QVBoxLayout(w); form = QtWidgets.QFormLayout()
        cfg = settings(); base = cfg.team_output_dir
        ed_in_root = QtWidgets.QLineEdit(str(base)); ed_in_root.setToolTip('Root directory with per-video embeddings (--in-root)')
        ...
        sb_k = QtWidgets.QSpinBox(); sb_k.setRange(2,32); sb_k.setValue(int(cfg.cluster_k)); sb_k.setToolTip('Number of clusters (--k)')
        form.addRow('K (--k)', sb_k)
        sb_umap_dim = QtWidgets.QSpinBox(); sb_umap_dim.setRange(2,512); sb_umap_dim.setValue(int(cfg.umap_dim)); sb_umap_dim.setToolTip('UMAP output dimensionality (--umap-dim)')
        form.addRow('UMAP dim (--umap-dim)', sb_umap_dim)
        sb_umap_neighbors = QtWidgets.QSpinBox(); sb_umap_neighbors.setRange(2,200); sb_umap_neighbors.setValue(int(cfg.umap_neighbors)); sb_umap_neighbors.setToolTip('UMAP n_neighbors (--umap-neighbors)')
        form.addRow('UMAP neighbors (--umap-neighbors)', sb_umap_neighbors)
        ed_umap_metric = QtWidgets.QLineEdit(str(cfg.umap_metric)); ed_umap_metric.setToolTip('UMAP distance metric (--umap-metric)')
        form.addRow('UMAP metric (--umap-metric)', ed_umap_metric)
        ds_umap_min_dist = QtWidgets.QDoubleSpinBox(); ds_umap_min_dist.setRange(0.0,1.0); ds_umap_min_dist.setSingleStep(0.05); ds_umap_min_dist.setValue(float(cfg.umap_min_dist)); ds_umap_min_dist.setToolTip('UMAP min_dist (--umap-min-dist)')
        ...
        sb_seed2 = QtWidgets.QSpinBox(); sb_seed2.setRange(0,10_000); sb_seed2.setValue(int(cfg.random_seed)); sb_seed2.setToolTip('Random seed (--seed)')
    ...
    def _make_eval_tab(self) -> QtWidgets.QWidget:
        ...
        form.addRow('Glob (--glob)', ed_glob)
        ed_video = QtWidgets.QLineEdit(str(root / 'data_hockey.mp4')); ed_video.setToolTip('Video file (--video)')
        ...
        sb_frame_step = QtWidgets.QSpinBox(); sb_frame_step.setRange(1,10_000); sb_frame_step.setValue(int(cfg.eval_frame_step)); sb_frame_step.setToolTip('Frame step (--frame-step)')
        ...
        ed_siglip = QtWidgets.QLineEdit(str(cfg.siglip_model)); ed_siglip.setToolTip('SigLIP model id (--siglip)')
        ed_yolo_model = QtWidgets.QLineEdit(str(cfg.yolo_model)); ed_yolo_model.setToolTip('YOLO detection model (--yolo-model)')
        sb_imgsz = QtWidgets.QSpinBox(); sb_imgsz.setRange(64,4096); sb_imgsz.setValue(int(cfg.yolo_imgsz)); sb_imgsz.setToolTip('YOLO inference size (--imgsz)')
        ds_conf = QtWidgets.QDoubleSpinBox(); ds_conf.setRange(0.0,1.0); ds_conf.setSingleStep(0.01); ds_conf.setValue(float(cfg.yolo_conf)); ds_conf.setToolTip('YOLO confidence (--conf)')
        sb_max_boxes = QtWidgets.QSpinBox(); sb_max_boxes.setRange(1,100); sb_max_boxes.setValue(int(cfg.yolo_max_boxes)); sb_max_boxes.setToolTip('Max boxes (--max-boxes)')
        ds_central = QtWidgets.QDoubleSpinBox(); ds_central.setRange(0.05,1.0); ds_central.setSingleStep(0.05); ds_central.setValue(float(cfg.central_ratio_default)); ds_central.setToolTip('Central crop ratio (--central-ratio)')
        ...
        form.addRow('Save grid (--save-grid)', cb_save_grid)
        sb_limit_images = QtWidgets.QSpinBox(); sb_limit_images.setRange(1,10_000); sb_limit_images.setValue(int(cfg.eval_limit_images)); sb_limit_images.setToolTip('Max annotated images (--limit-images)')
