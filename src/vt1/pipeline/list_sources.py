...
def probe_camera_index(i: int, timeout=1.0):
    ...
    for be in backends:
        ...
        if ok and frame is not None:
            ...
            return {
                "index": i,
                "backend": "CAP_DSHOW" if be == cv2.CAP_DSHOW else ("CAP_MSMF" if be == cv2.CAP_MSMF else "DEFAULT"),
                "width": w,
                "height": h,
                "fps": float(fps),
            }
...
def probe_video_file(p: Path, timeout=1.0):
    ...
    h = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT)) or frame.shape[0]
    fps = cap.get(cv2.CAP_PROP_FPS) or 0.0
    count = int(cap.get(cv2.CAP_PROP_FRAME_COUNT)) if cap.get(cv2.CAP_PROP_FRAME_COUNT) > 0 else 0
...
def get_windows_camera_names():
    ...
    try:
        ...
        out = subprocess.check_output(["powershell", "-NoProfile", "-Command", ps], stderr=subprocess.DEVNULL, timeout=3)
...
def main():
    ...
    ap.add_argument("--max-index", type=int, default=10, help="Max webcam index to probe")
    ap.add_argument("--scan-dir", type=str, default=".", help="Directory to scan for video files")
    ap.add_argument("--no-recursive", action="store_true", help="Disable recursive scan for video files")
    ap.add_argument("--timeout", type=float, default=1.0, help="Seconds to wait when grabbing a frame")
    args = ap.parse_args()
    ...
    if cams:
        for c in cams:
            print(f"- Camera index {c['index']} [{c['backend']}], {c['width']}x{c['height']}, fps~{c['fps']:.1f}  -> use --source {c['index']}")
    else:
    ...
    print(f"Scanning video files under {root} ({'non-recursive' if args.no_recursive else 'recursive'})...")
    ...
    if vids:
        for v in vids:
            ...
            print(f"- File {v['path']}  ({v['width']}x{v['height']}, fps~{v['fps']:.1f}{dur})  -> use --source {v['path']}")
    else:
    ...
    print("- Network streams supported by OpenCV/FFmpeg, e.g. rtsp://user:pass@host:554/stream (if available)")
