...
class PipelineTab(QtWidgets.QWidget):
    ...
    def _build_ui(self):
        ...
        form.addRow(
            "Video source (--source)", self._hrow(self.ed_source, self.btn_source)
        )
        self.ed_pose_model = QtWidgets.QLineEdit()
        self.ed_pose_model.setToolTip(
            "Ultralytics YOLO pose model path/name (--pose-model)"
        )
        ...
        form.addRow(
            ...
            self._hrow(self.ed_pose_model, self.btn_pose_model),
        )
        ...
        self.sb_sam_reinit.setToolTip(
            "Re-init SAM every N frames (0=never) (--sam-reinit)"
        )
        ...
        form.addRow(
            "Empty CUDA cache every N (--empty-cache-interval)", self.sb_empty_cache
        )
        self.ed_metrics = QtWidgets.QLineEdit()
        self.ed_metrics.setToolTip(
            "Write per-run metrics JSON to this path if set (--metrics-json)"
        )
        self.btn_metrics = QtWidgets.QPushButton("Browse…")
        self.btn_metrics.setToolTip("Select metrics JSON output path")
        form.addRow(
            "Metrics JSON (--metrics-json, optional)",
            self._hrow(self.ed_metrics, self.btn_metrics),
        )
        self.ed_team_models = QtWidgets.QLineEdit()
        self.ed_team_models.setToolTip(
            "Directory containing umap.pkl and kmeans.pkl (--team-models)"
        )
        ...
        form.addRow(
            "Team models dir (--team-models)",
            self._hrow(self.ed_team_models, self.btn_team_models),
        )
        ...
        self.cb_disable_team = QtWidgets.QCheckBox(
            "Disable team coloring (--disable-team)"
        )
        self.cb_disable_team.setToolTip(
            "Disable team coloring even if models are present"
        )
    ...
    def _fill_defaults(self):
        cfg = settings()
        root = cfg.repo_root
        self.ed_source.setText(str(cfg.default_video_source))
    ...
    def _on_run(self):
        if self.proc is not None:
            QtWidgets.QMessageBox.warning(
                self, "Already running", "A process is already running. Stop it first."
            )
            return
        src = self.ed_source.text().strip()
        if not src or not Path(src).exists():
            QtWidgets.QMessageBox.warning(
                self, "Source required", "Please pick a valid video source file."
            )
            return
        ...
        # Use existing pipeline script under offline_pipeline
        script = str(self._repo_root() / "vt1" / "pipeline" / "sam_offline.py")
    ...
    def _build_args(self) -> list[str]:
        ...
        if self.sb_max_frames.value() > 0:
            ...
            args.append(str(self.sb_max_frames.value()))
